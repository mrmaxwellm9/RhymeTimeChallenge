...
def ensure_values_as_keys(combined_dict):
    ...
    for key, value in combined_dict.items():
        for v in value:
            ...
            if v not in combined_dict:
                invalid_values.append((key, v))
            # Check if the original key is present in the list of values for the value
            elif key not in combined_dict[v]:
                combined_dict[v].append(key)  # Add the original key to the list of values for the value
...
# Output the updated combined dictionary to a file
with open('updated_combined_dictionary.txt', 'w') as file:
    for key, value in combined_dict.items():
        file.write(f"{key}: {value}\n")
